        ...
        for attempt in range(3):
            try:
                ...
                if "429" in emsg or "RESOURCE_EXHAUSTED" in emsg:
                    match = re.search(r"retry[^0-9]*([0-9]+(?:\.[0-9]+)?)\s*s", emsg, re.I)
                    wait = float(match.group(1)) if match else 30.0
                    wait = min(wait, 60.0)
                    print(f"[search_catalog_rag] Rate limit hit — waiting {wait:.0f}s (attempt {attempt+1}/3)")
                    time.sleep(wait)
                    last_exc = e
                else:
                    raise
            ...
            raise RuntimeError(
                f"Rate limit retries exhausted after 3 attempts. "
                f"Switch to Claude in Settings or wait and retry. Last error: {last_exc}"
            )
        result = _select_tanda(bundle, df)
        if result and result.tanda:
            return [t for t in result.tanda]
